...
2. Sieć lokalna - Taka sieć jest dużo mniejsza niż sieć globalna i ogranicza się do urządzeń stojących za pojedynczym routerem. Lokalny ruch sieciowy nie wydostaje się do sieci publicznej poprzez router. To właśnie poprzez sieć lokalną przechodzi ruch sieciowy podczas gry LAN czy komunikacji z drukarką.
...
3. Zmień opcje w configu NanoLimbo:
   * bind.port: Port który chcesz użyć dla serwera Limbo
   * bind.ip: Adres który chcesz użyć dla serwera Limbo
   * infoForwarding.type: MODERN / BUNGEE\_GUARD, zależnie co wybrałeś wcześniej
   * infoForwarding.secret / tokens: Klucz serwera proxy
4. Dodaj serwer NanoLimbo do configu serwera proxy tak samo jak pozostałe serwery
5. Wrzuć LibreLogin do folderu 'plugins' w plikach serwera Velocity
6. Otwórz plik 'config.conf' w folderze 'plugins/LibreLogin'
7. Zmień opcje:
   * auto-register=true
   * new-uuid-creator=MOJANG
8. Wpisz swoje serwery lobby oraz Limbo w odpowiednie miejsca
9. Skonfiguruj przedział portów dla serwera Limbo w pliku konfiguracyjnym LibreLoginu tak, aby mieścił się w nim port twojego serwera Limbo
10. Uruchom Velocity
...
**AKTUALNIE NIC TU NIE MA, ROZBUDOWANY PORADNIK MOŻE POJAWI SIĘ W PRZYSZŁOŚCI.** Przykładowe usługi filtrów zewnętrznych:
